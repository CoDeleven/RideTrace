...
fun put(url:String, mockMvc:MockMvc, content:Any):String{
    return mockMvc.perform(MockMvcRequestBuilders
            .put(url).contentType(MediaType.APPLICATION_JSON_UTF8).content(ObjectMapper().writeValueAsString(content))).andExpect(MockMvcResultMatchers.status().isOk).andReturn().response.contentAsString
}

fun get(url:String, mockMvc:MockMvc, params:Map<String, List<String>>?):String{

    return mockMvc.perform(MockMvcRequestBuilders
            .get(url).params(LinkedMultiValueMap(params))).andExpect(MockMvcResultMatchers.status().isOk).andReturn().response.contentAsString
}
